...
void main() {
	...
	void Shader_Yuva2Rgba::Draw(uint8_t const* const& yData, uint8_t const* const& uData, uint8_t const* const& vData, uint8_t const* const& aData, uint32_t const& yaStride, uint32_t const& uvStride, uint32_t const& w, uint32_t const& h, XY const& pos) {
		...
		glTexImage2D(GL_TEXTURE_2D, 0, GL_LUMINANCE, yaStride, h, 0, GL_LUMINANCE, GL_UNSIGNED_BYTE, yData);
		CheckGLError();

		auto texU = LoadGLTexture_core(1);
		glTexImage2D(GL_TEXTURE_2D, 0, GL_LUMINANCE, uvStride, h / 2, 0, GL_LUMINANCE, GL_UNSIGNED_BYTE, uData);
		CheckGLError();

		auto texV = LoadGLTexture_core(2);
		glTexImage2D(GL_TEXTURE_2D, 0, GL_LUMINANCE, uvStride, h / 2, 0, GL_LUMINANCE, GL_UNSIGNED_BYTE, vData);
		CheckGLError();

		auto texA = LoadGLTexture_core(3);
		glTexImage2D(GL_TEXTURE_2D, 0, GL_LUMINANCE, yaStride, h, 0, GL_LUMINANCE, GL_UNSIGNED_BYTE, aData);
		CheckGLError();
